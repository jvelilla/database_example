	...
	read_boolean (a_index: INTEGER): detachable BOOLEAN
		...
		do
			...
			if attached {DB_TUPLE} item as l_item then
				...
				elseif attached {BOOLEAN_REF} l_item.item (a_index) as ll_item then
					Result := ll_item.item
					-- MySQL special use case.
				elseif attached {INTEGER_32_REF} l_item.item (a_index) as ll_item  then
					Result := ll_item.item.to_boolean
				elseif attached {INTEGER_16_REF} l_item.item (a_index) as ll_item  then
					Result := ll_item.item.to_boolean
				end
			end
		end
